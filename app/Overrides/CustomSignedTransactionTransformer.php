...
class CustomSignedTransactionTransformer implements CanPack, MessagePackTransformer
{
    private CustomTransformerFactory $factory;

    /**
     * @param CustomTransformerFactory|null $factory
     */
    public function __construct(?CustomTransformerFactory $factory = null)
    {
        $this->factory = $factory ?? new CustomTransformerFactory();
    }
    ...
    public function transform(string $className, array $value, ?array $extraData = null)
    {
        $type = $extraData['type'];
        $authAddress = EncoderUtils::toAddress($extraData['sgnr']);
        $signature = EncoderUtils::toSignature($extraData['sig']);
        $txn = $value;
