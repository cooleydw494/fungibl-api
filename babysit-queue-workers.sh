...
worker_status=$(sudo supervisorctl status fungibl-worker | awk '{print $2}')
...
log_file="/www/fungibl-api/storage/logs/worker.log"
...
if [[ $(date -d "$processing_timestamp" +%s) -lt $(date -d "15 minutes ago" +%s) ]]; then
    sudo supervisorctl restart fungibl-worker:*
fi
